...
3. 크기 제한 없음, 타입제한 없음
         크기      데이터의 타입
배열    변경불가     선언된 한가지 타입만 저장 가능
리스트  변경불가      다양한 데이터 타입 저장가능
...
1. new_list = old_list
# 주소의 복사, 얕은 복사
# 수행은 빠르지만 복사하는 큰 의미가 없다

2. new_list = old_list[:]
# 슬라이싱하여 새로운 객체를 얻는 복사, 깊은 복사 중 가장 빠른 방법
...
# 시퀀스 자료형을 리스트 객체로 변환해주는 복사, 깊은 복사

5. import copy
   new_list = copy.copy(old_list)
# copy 활용, 깊은 복사

6. new_list = [i for i in old_list]
# 리스트함축, 깊은 복사

7. import copy
   new_list = copy.deepcopy(old_list)
# 복사하는 리스트의 원소의 객체까지도 복사
# 가장 느린 깊은 복사
...
[0x1000(Head)] -> [0x1000(A)|0x1008] -> [0x1008(B)|0x1010] -> [0x1010(C)|None]


단순 연결 리스트의 삽입 연산
'A', 'C', 'D'를 원소로 갖고 있는 리스트의 두 번째에 'B'노드를 삽입할 때
1) 메모리를 할당하여 새로운 노드 new 생성  :  new -> 0x1018()
2) 새로운 노드 new의 데이터 필드에 'B'저장  :  new -> 0x1018(B)
3) 삽입될 위치의 바로 앞에 위치한 노드의 링크 필드를 new에 복사
[0x1000(Head)] -> [0x1000(A)|0x1010] -> [0x1010(C)|None]
                     new -> [0x1008(B)|0x1010]
4) new의 주소를 앞 노드의 링크 필드에 저장
[0x1000(Head)] -> [0x1000(A)|0x1018] -> [0x1010(C)|None]
                     new -> [0x1008(B)|0x1010]
...
[0x1000(Head)] -> [0x1000(A)|0x1008] -> [0x1008(B)|0x1010](삭제할 노드) -> [0x1010(C)|None]
2) 삭제할 노드의 링크 필드를 선행노드의 링크 필드에 복사
[0x1000(Head)] -> [0x1000(A)|0x1010] -> [0x1010(C)|None]
                  Target -> [0x1008(B)|0x1010](삭제할 노드)
결과적으로 B를 가리키는 노드가 아무것도 없기 떄문에 B가 삭제된 것과 같은 효과
...
5) new의 값을 new가 가리키는 노드의 다음 노드의 prev 필드에 저장하여
   삽입하려는 노드의 다음 노드와 삽입하려는 노드를 연결
